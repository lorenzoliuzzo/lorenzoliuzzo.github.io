---
title: "About Me"
excerpt: "Physicist and developer specialized in Artificial Intelligence, combining Machine Learning expertise with robust Software Engineering in order to solve complex problems."
permalink: /about/

layout: single
classes: wide
toc: true
---

<p class="lead">
  Physicist and developer working on Artificial Intelligence, combining machine
  learning with software engineering to solve problems in the physical sciences.
</p>

<p class="cta-row">
  <a href="/assets/cv/cv_liuzzo_en.pdf" class="btn btn--primary">Curriculum Vitae (EN)</a>
  <a href="/assets/cv/cv_liuzzo_it.pdf" class="btn btn--inverse">Versione italiana</a>
</p>

## Interests

<dl class="definition-list">
  <dt>Scientific Machine Learning</dt>
  <dd>Applying ML to problems in physics and astronomy, where the model has to respect the physics rather than just fit the data.</dd>

  <dt>Computational Physics &amp; Simulation</dt>
  <dd>Ray tracing, Monte Carlo methods and optical systems modelling.</dd>

  <dt>Software Architecture</dt>
  <dd>Building robust, scalable systems for data-intensive scientific work.</dd>
</dl>

## Education

<ol class="timeline">
  <li class="timeline__item">
    <p class="timeline__period">2025 &mdash; present</p>
    <h3 class="timeline__title">MSc in Artificial Intelligence for Science and Technology</h3>
    <p class="timeline__place">University of Milan &middot; Milano-Bicocca &middot; Pavia</p>
  </li>
  <li class="timeline__item">
    <p class="timeline__period">2021 &mdash; 2025</p>
    <h3 class="timeline__title">BSc in Physics</h3>
    <p class="timeline__place">University of Milan <span class="timeline__note">100/110</span></p>
  </li>
  <li class="timeline__item">
    <p class="timeline__period">2016 &mdash; 2021</p>
    <h3 class="timeline__title">Scientific Diploma</h3>
    <p class="timeline__place">I.I.S. Severi-Correnti, Milan <span class="timeline__note">100/100</span></p>
  </li>
</ol>

## Technical Skills

<div class="skill-groups">
  <div class="skill-group">
    <h3 class="skill-group__title">Languages</h3>
    <ul class="chip-list">
      <li>C++</li><li>Python</li><li>Julia</li><li>C</li><li>Nim</li>
    </ul>
  </div>
  <div class="skill-group">
    <h3 class="skill-group__title">ML &amp; Data</h3>
    <ul class="chip-list">
      <li>PyTorch</li><li>Scikit-learn</li><li>Pandas</li><li>NumPy</li><li>SciPy</li>
    </ul>
  </div>
  <div class="skill-group">
    <h3 class="skill-group__title">Tools</h3>
    <ul class="chip-list">
      <li>Git</li><li>GitHub</li><li>Jupyter</li><li>LaTeX</li><li>Typst</li>
    </ul>
  </div>
</div>

## Contacts

The best way to reach me is [email](mailto:lorenzoliuzzo@outlook.com). I'm also on
[GitHub](https://github.com/lorenzoliuzzo), where I host the code behind the
coursework and side projects listed under [Projects](/projects/).
